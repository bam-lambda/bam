...
const getLambdaNamesFromAws = async () => {
  ...
  const functionNames = functionsObjects.Functions.map(
    (functionObj) => functionObj.FunctionName,
  );
...
const formatBamFunctionsList = async (funcName, lambdas, apis, region) => {
  ...
  if (Object.keys(lambdas[region]).length > 0) {
    ...
    const descriptionStr = `${indentFurthest}${bamText('description:')} ${
      funcObj.description
    }`;
    ...
    if (Object.keys(apis[region]).length > 0) {
      ...
      if (apiExistsOnAws) {
        ...
        const endpointStr = `${indentFurthest}${bamText('endpoint:')} ${
          apiObj.endpoint
        }`;
        const methodsStr = methods.join(', ');
        const httpMethodsStr = `${indentFurthest}${bamText(
          'http methods:',
        )} ${methodsStr}`;
...
const getBamFunctionsList = async (path, lambdas, apis, region) => {
  const localFunctionsAlsoOnAws = await getLocalFunctionsAlsoOnAws(
    path,
    lambdas,
    region,
  );
  ...
  for (let i = 0; i < localFunctionsAlsoOnAws.length; i += 1) {
    ...
    const listItem = await formatBamFunctionsList(
      funcName,
      lambdas,
      apis,
      region,
  ...
  const functionsList = functionsListItems.join(
    `${vertPadding}${indentFurther}`,
  );
  return numOfLocalFunctionsAlsoOnAws === 0
    ? ''
    : `${indentFurther}${functionsList}`;
};

const formatAwsFunctionsList = (awsFuncs) =>
  awsFuncs.map((funcName) => `${indentFurther}${funcName}`).join('\n');
...
const getAwsFunctionsList = async (path, lambdas, region) => {
  ...
  const functionsOnlyOnAws = functionsOnAws.filter(
    (funcName) => !localFunctionsAlsoOnAws.includes(funcName),
  );
  const numOfFunctionsOnlyOnAws = functionsOnlyOnAws.length;
  return numOfFunctionsOnlyOnAws === 0
    ? ''
    : formatAwsFunctionsList(functionsOnlyOnAws);
};
...
const formatTablesList = async (tableName, dbtables, region) => {
  ...
  const partitionKeyStr = `${indentFurthest}${bamText('partition key:')} ${
    partitionKey.name
  ...
  if (sortKey) {
    ...
    const sortKeyStr = `${indentFurthest}${bamText('sort key:')} ${
      sortKey.name
    } (${sortKeyDataType})`;
...
const getBamTablesList = async (path, dbtables, region) => {
  ...
  const tableNames = Object.keys(regionalTables).filter((table) =>
    tablesOnAws.includes(table),
  );
  const numOfTables = tableNames.length;
  const formattedTablesList = await getFormattedTablesList(
    tableNames,
    dbtables,
    region,
  );
